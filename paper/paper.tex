...
This paper pursues the complementary path. We keep the interpretable, feature-based method, reproduce it on openly licensed data so the results can be checked and reused, and add the apparatus a verification claim requires. Our contributions are: (1) a reproduction of the feature pipeline of \textcite{clerice2023} on open corpora; (2) a calibrated, \emph{open-set} verification protocol -- Bootstrap Distance Impostors \cite{nagy2024} equipped with a conformal reject option \cite{vovk2005,angelopoulos2023} that gives each verdict distribution-free coverage and lets the method abstain when the true author is absent, a per-channel decomposition of the evidence, and a provenance control that keeps shared digitization from being read as shared authorship; and (3) an evaluation against the published \textsc{cpg} attributions, on which the method recovers the true author for every ground-truth group it has a candidate for and, for the anonymous majority, supplies confidence-graded proposals whose linguistic basis can be inspected.
