...
\title{Interpretable, Calibrated Authorship Verification of the Pseudo-Chrysostoms on Open Greek Data}
...
\begin{abstract}
More than fifty homilies of Severian of Gabala survive in Greek only because scribes copied them under the name of his rival, John Chrysostom; recovering such attributions is a long-running task of patristic philology, already addressed once with a neural verifier. We revisit it with an interpretable, calibrated, reproducible protocol built entirely on openly licensed Greek corpora and on hand-crafted features -- function words, part-of-speech trigrams, and character affixes. To a reproduced feature pipeline we add: a document-grouped evaluation that removes a within-document leakage which otherwise inflates verification scores; a Bootstrap Distance Impostors verifier read as a calibrated likelihood and wrapped in a conformal layer with a distribution-free coverage guarantee; a provenance control that separates authorship from shared digitization; and a per-channel decomposition that exposes the linguistic evidence behind each verdict. Applied to twenty-two groups of pseudo-Chrysostomica, the calibrated verifier recovers the published attribution for every ground-truth group whose author is a candidate -- among them the one genuinely Chrysostomic group, which a nearest-author baseline misassigns to Severian. We also mark the limits the open corpus imposes: several candidate authors are represented by a single document, so the conformal prediction sets are wide and the reject option only partial; and a companion fragment-level analysis, validated at AUC $0.9$, finds most homilies internally coherent, with a short list of composite candidates. Throughout, neither a retrained Siamese metric nor a frozen contextual encoder improves on the transparent features.
\end{abstract}
...
Our study stands closest to \textcite{clerice2023}, and it is worth marking the overlap plainly. That work targets the same corpus, represents each text with the same three channels -- most frequent words, character affixes, and part-of-speech trigrams -- trains a Siamese network with a signal-to-noise-ratio distance (AUC-ROC $0.845$), and grades its verdicts against Voicu's twenty-one groupings. We reuse those features and, as a cross-check, a reproduced network of the same kind; what we add is the apparatus that makes a verification claim auditable and calibrated. The reference is rebuilt entirely from openly licensed corpora, so the pipeline can be re-run; every same-author comparison is document-grouped, after we found that a work's own segments inflate naive scores; each verdict carries a calibrated likelihood and a conformal coverage guarantee; a provenance control separates authorship from the shared digitization of the sources; and a per-channel decomposition, with a table of the words that most distinguish the rival authors, exposes the linguistic basis of every judgement. The neural verifier of \textcite{clerice2023} is a corroborating instrument here, not the method under test.

This paper pursues the complementary path. We keep the interpretable, feature-based method, reproduce it on openly licensed data so the results can be checked and reused, and add the apparatus a verification claim requires. Our contributions are: (1) a reproduction of the feature pipeline of \textcite{clerice2023} on open corpora, with a document-grouped evaluation that corrects a within-document leakage; (2) a calibrated verification protocol -- Bootstrap Distance Impostors \cite{nagy2024} read as a likelihood and wrapped in a conformal layer \cite{vovk2005,angelopoulos2023} that attaches distribution-free coverage to each verdict -- together with a provenance control and a per-channel decomposition of the evidence; (3) an evaluation against the published \textsc{cpg} attributions, on which the method recovers the true author for every ground-truth group whose author is a candidate; and (4) a fragment-level composite-authorship detector, validated on the reference, that flags which transmitted homilies may not be single-hand. Two neural cross-checks -- a retrained Siamese metric and a frozen contextual encoder used as an extra channel -- match the transparent features without beating them, evidence that the interpretable representation is not the limiting factor.

\section{Data}

\paragraph{Reference corpus.} We assemble a reference of clean, critically edited Greek from two open sources: the Open Greek and Latin \emph{First1KGreek} project \cite{first1k} and the Patristic Text Archive \cite{pta}. After repairing author labels and balancing the corpus so that no writer dominates (each author capped at 30 works, singletons removed), the reference holds 1{,}082 text segments drawn from 374 distinct works by 113 authors. The candidate patristic authors include Severian of Gabala and John Chrysostom, together with Eusebius, Cyril of Alexandria, Theodoret, Athanasius, and Origen; Severian is represented by the critically edited \emph{Homiliae Pseudo-Chrysostomicae} \cite{hpc}. Coverage is uneven, and it bears on what follows: Severian is present as 26 distinct works and Athanasius as 8, but John Chrysostom and Theodoret survive here as a single work each (segmented into a few passages). Verdicts that rest on those two authors are correspondingly provisional -- a direct cost of working from open corpora rather than the licensed Thesaurus Linguae Graecae.
...
\paragraph{Document-grouped evaluation.} The reference is segmented: each work is split into passages of a few hundred tokens. Two segments of one work resemble each other far more than two works by the same author, so pooling a work's own segments into its target set inflates same-author matching -- on 80 reference works with a segmented sibling, including the siblings raises the same-author impostors score by $0.13$ on average (from $0.66$ to $0.79$). Every same-author comparison in this paper is therefore document-grouped: a query is scored only against its author's \emph{other} works, never against its own siblings. This lowers the headline numbers below their leaky values but is the only defensible way to measure cross-document authorship on a segmented corpus.
...
Before applying the verifier to disputed texts, we measure it on labelled reference authors. From the 45 authors with at least five works and at least two distinct documents we build 270 document-grouped same/different-author verification problems and score each with three independent feature-space methods: the impostors win-fraction used here, Burrows's Delta to an author centroid, and cosine similarity. All three separate the classes well (Table~\ref{tab:benchmark}, Figure~\ref{fig:roc}); the impostors method is the most accurate (ROC-AUC $0.961$, $F_1$ $0.891$), and the three scores agree closely (Spearman $\rho$ between $0.80$ and $0.94$). These figures are lower than the $\approx\!0.998$ obtained when a work's own segments are left in the target set, but they measure genuine cross-document authorship rather than within-document similarity. The verdicts that follow are accurate and robust to the choice of scorer.

\begin{table}[t]
  \centering
  \small
  \input{tables/benchmark.tex}
  \caption{Document-grouped verification benchmark on 270 problems from the reference authors (each query is scored only against its author's \emph{other} works). The impostors method is the most accurate, and Delta and cosine corroborate it.}
  \label{tab:benchmark}
\end{table}
...
\paragraph{Calibration.} On held-out reference works the verifier separates same-author from different-author cases sharply: genuine Severian works -- verified against Severian's \emph{other} documents -- score a mean BDI of $0.86$, other authors $0.09$ (Figure~\ref{fig:calibration}). Scores are therefore interpretable on an absolute scale, and they are stable: across five random seeds the standard deviation of a group's score is $0.00$--$0.04$. Figure~\ref{fig:calibration} also shows that most pseudo-Chrysostom groups fall in the intermediate zone between the two reference distributions -- neither confidently Severian nor confidently foreign.
...
\subsection{Conformal prediction sets and the limits of the evidence}

The verdicts so far are point estimates. Wrapping the impostors score in split-conformal prediction attaches a coverage guarantee and, in principle, the option to answer ``not in this set of candidates''; on this corpus it also measures how far the reference's thin per-author depth limits confident verification. On held-out reference works whose author \emph{is} a candidate, the $90\%$ prediction set contains the true author $90\%$ of the time and the $80\%$ set $80\%$ of the time -- coverage is exact by construction -- but with document-grouped calibration the sets are wide (mean size $2.3$ at $\alpha=0.10$). For works whose author is \emph{not} a candidate, the set is empty $60\%$ of the time at $\alpha=0.10$ and $84\%$ at $\alpha=0.20$: the reject option fires, but only in part.

Applied to the pseudo-Chrysostoms at $\alpha=0.10$ (Table~\ref{tab:conformal}), the verifier resolves only three groups to a single author and returns a multi-candidate set for most of the rest, with Severian -- the reference's deepest homiletic author -- present in nearly every set. Where scholarship names a candidate that is in the room, the set contains it: PC9's set is \{Severian, Cyril\}, PC21's \{Severian, Chrysostom\}, and PC1's admits John Chrysostom among four. The wide sets report the state of the evidence: on this corpus the calibrated method supports \emph{ranking} candidates far more than naming one, and it declines to pretend otherwise.

That the sets are this wide follows directly from removing the within-document leakage. On segmented data the naive calibration is over-confident; once a query is scored only against an author's \emph{other} works, the reference's thin coverage -- five of the seven candidates have four or fewer distinct works, and two have only one -- leaves little to separate them. Conformal prediction does not manufacture confidence the data lack; here it quantifies their absence. The same protocol on a corpus with dozens of works per author would tighten the sets, which is a statement about the data rather than the method.

\begin{table}[t]
  \centering
  \small
  \input{tables/conformal.tex}
  \caption{Conformal $90\%$ prediction sets for selected pseudo-Chrysostom groups under document-grouped calibration. The sets are wide -- the method ranks candidates rather than naming one -- but where scholarship supplies a candidate in the room (PC9, PC21, PC1) the set contains it.}
  \label{tab:conformal}
\end{table}
...
\paragraph{A frozen neural channel adds nothing.} A reviewer might still suspect that a modern contextual encoder would beat the hand-crafted features. It does not. Taking the Ancient-Greek BERT that produced our part-of-speech tags \cite{superpeitho} as a frozen document encoder -- inference only, no fine-tuning -- we mean-pool a $768$-dimensional embedding per text and treat it as a fourth verification channel. On the same $270$ document-grouped benchmark problems it verifies authorship slightly \emph{less} accurately than the transparent features (Table~\ref{tab:bert}): impostors on the neural embedding reach ROC-AUC $0.946$, against $0.963$ for the function-word, part-of-speech and affix channels combined. The two agree closely (Spearman $0.87$ across the benchmark), and on the pseudo-Chrysostoms they return the same leading candidate for nineteen of the twenty-two groups, including every ground-truth case -- PC1 to Chrysostom, PC9 and PC21 to Severian. Where they part (PC4, PC19) the neural channel leans to Severian, the corpus's dominant author, which is the pull calibration is meant to resist. A $768$-dimensional black box thus buys nothing over a thousand function words and some affixes -- it costs a little -- and it corroborates rather than overturns the interpretable verdicts.

\begin{table}[t]
  \centering
  \small
  \input{tables/bert.tex}
  \caption{A frozen Ancient-Greek BERT, used inference-only as a fourth channel, on the $270$-problem document-grouped benchmark. Impostors on the neural embedding do not improve on the hand-crafted channels.}
  \label{tab:bert}
\end{table}
...
\subsection{Detecting composite homilies}

The segment structure that inflated the naive scores also enables a sharper question than the group-level cohesion above: is a single transmitted homily the work of one hand? A genuinely single-author text has internally coherent fragments; a composite one -- a florilegium, an interpolated core, or two hands -- does not. Splitting each reference document into fixed-size fragments and recomputing the interpretable function-word and affix features, we find that internal coherence separates genuine single-author texts from synthetic cross-author composites at AUC $0.96$; and, controlling for the mixing itself by pitting same-author against different-author fragment mixtures, at AUC $0.89$ -- so the signal is authorship, not merely shared topic. Two length effects are handled directly: fragments are a fixed size, so a shorter fragment's noisier features cannot pass for incoherence, and each homily's coherence is scored against a null of genuine single-author documents matched on fragment count.

Applied to the pseudo-Chrysostoms, the detector finds most assessable homilies internally coherent. Of the 46 homilies long enough to yield at least two fragments, only a handful are more dispersed than a typical single-author text of the same length (Table~\ref{tab:composite}); the clearest -- though each rests on only two fragments and is therefore provisional -- are \emph{In decem virgines} (PC3), \emph{In venerabilem crucem} (PC13), and \emph{In centurionem} (PC9). These are candidates for philological scrutiny, not verdicts. The broader result runs the other way: at fragment level these homilies are largely single-hand, so the pseudo-Chrysostom problem here is one of whole-text misattribution more than internal compilation.

\begin{table}[t]
  \centering
  \small
  \input{tables/composite.tex}
  \caption{Pseudo-Chrysostom homilies whose fragments are more internally dispersed than a genuine single-author text of the same fragment count $k$; $p$ is the fraction of the single-author null at least as dispersed. Two-fragment cases are provisional. ``Fragment lean'' gives each fragment's nearest candidate.}
  \label{tab:composite}
\end{table}
...
What the open corpus cannot do is equally plain. Because several candidate authors are present as a single document, the calibrated verifier ranks more often than it names: its conformal sets stay wide and its reject option is partial. This is a readout of the evidence, not a fault of the method -- the same protocol on the licensed Thesaurus Linguae Graecae, with dozens of works per author, would tighten both. The fragment-level detector points the same way, finding these homilies largely single-hand and locating the difficulty in whole-text transmission rather than internal compilation.

\paragraph{Limitations.} The reference, though open, is a single assemblage, and its per-author depth is uneven: two candidate authors (Chrysostom, Theodoret) are each present as a single work, so verdicts involving them rest on thin evidence, and author families it lacks (the Anomoean PC12) cannot be found at all -- a boundary of any closed-candidate method. Affix-driven verdicts may reflect orthographic normalisation in the editions rather than authorship, a caveat the per-channel view makes visible but does not remove. The two Severian-positive ground-truth attributions (PC9's \textsc{cpg} 4215; PC21's \textsc{cpg} 4564) rest on the current editorial consensus, itself in part still argued; our agreement corroborates but does not settle them. Finally, the proposals for the anonymous groups, and the composite candidates, are starting points for philological work, not conclusions.

\section{Conclusion}

Interpretable, feature-based stylometry remains a serviceable instrument for disputed patristic authorship when it is equipped to make -- and to explain, and to qualify -- a verification claim. On open Greek data, a calibrated, document-grouped protocol reproduces the published attribution of the pseudo-Chrysostoms where scholarship provides one, identifies the single genuinely Chrysostomic group without prompting, and offers per-channel proposals for the remainder; where the corpus is too thin to separate candidates, its conformal sets stay wide rather than feign confidence, and a fragment-level test finds the transmitted homilies largely single-hand. That neither a retrained Siamese nor a frozen encoder improves on the hand-crafted features shows the transparent representation is not the limiting factor -- the data are. The result is not a black box that pronounces, but an instrument a philologist can interrogate, and one that reports the reach of its own evidence.
